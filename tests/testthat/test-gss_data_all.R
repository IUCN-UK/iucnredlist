test_that("gss_data_all calls perform_request and returns expected response", {
  httptest2::with_mock_dir("gss_data_all", {
    ...
    expect_true(is.list(raw_gss_result))
    expect_length(raw_gss_result, 1)
    expect_length(unique(raw_gss_result$assessments), 115) # 115 assessments returned

    expect_equal(names(raw_gss_result), c("assessments"))

    expect_equal(names(raw_gss_result$assessments[[1]]), c(
